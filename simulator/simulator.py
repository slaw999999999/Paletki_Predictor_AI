...
class FindValueTest(ValueEngine):
    def __init__(self):
        ...
        self.gb_model = joblib.load("/data/paletki/trained_model_xgb_away.pkl")

        # Ustalone progi pewności (prawdopodobieństwo z predict_proba)/ bazowane na results_models.txt. Cel to minimum 80% accuracy
        self.threshold_away = 0.71  # jeśli p1 >= 0.67 => bet na gościa (klasa 1)
    ...
    def find_value_two(self, date_for_match, home, away, elo_home, elo_away, odd_home, odd_away,
                        ...
                        diff_saved,diff_wasted,diff_walecznosc_3,combined_form_chart,actual_form_home,actual_form_away):
        # Generowanie wspólnego wykresu formy
        #jest juz wywolywane wczesniej
        ...
        diff_last_tournaments = diff_last_tournaments * -1
        diff_zmeczenie = diff_zmeczenie * -1
        diff_h2h = diff_h2h * -1
        diff_players_vs_opponents_50 = diff_players_vs_opponents_50 * -1
        diff_dominance_percentage = diff_dominance_percentage * -1
        diff_closest_trends =  diff_closest_trends *-1
        diff_saved = diff_saved * -1
        elo_diff_scaled = elo_diff_scaled * -1


        h2h_away_scaled = 1 - h2h_home_scaled
        ...
        data_dict = {
            ...
            "h2h_away_scaled": h2h_away_scaled,
            "diff_h2h": diff_h2h,
        ...
        p0, p1 = float(proba_gb[0]), float(proba_gb[1])
    ...
    async def add_experience(self, betting_collection):
        matches = betting_collection.find()

        for record in matches:
            self.match = record
            combined_form_chart, home_cumulative_elo, away_cumulative_elo = self.get_combined_form_chart(record["home_name"], record["away_name"])
            experience = self.compare_experience(home_cumulative_elo, away_cumulative_elo)
            betting_collection.update_one({"_id": record["_id"]}, {"$set": {"experience": experience}})
        
        print("done")

    async def process_matches(self, betting_collection, n, create_features=False):
        """
        Process matches asynchronously to avoid using `asyncio.run` within the event loop.
        """
        if create_features:
            matches = self.get_upcoming_matches(n, True) #n - liczba godzin do przodu
        else:
            matches = self.get_upcoming_matches(n) #n - liczba godzin do przodu
        ...
        for record in matches:
            ...
            if create_features:
                ...
                home_id = list(self.players_coll.find({"player_name": record["home_name"]}, {'player_id': 1}))
                away_id = list(self.players_coll.find({"player_name": record["away_name"]}, {'player_id': 1}))
                if not home_id or not away_id:
                    continue
                else:
                    record["home_id"] = home_id[0]["player_id"]
                    record["away_id"] = away_id[0]["player_id"]
            ...
            if home_name_tt != '' and away_name_tt != '':
                ...
                combined_form_chart, home_cumulative_elo, away_cumulative_elo = self.get_combined_form_chart(home_name_tt, away_name_tt)
                experience = self.compare_experience(home_cumulative_elo, away_cumulative_elo)
            ...
            if not create_features:
                ...
                if home_name_tt and away_name_tt:
                    ...
                    result_one = self.find_value_two(date_for_match, home_name_tt, away_name_tt, elo_home, elo_away, odd_home, odd_away,
                                                    ...
                                                    diff_players_vs_opponents_50,diff_dominance_percentage,diff_closest_trends,
                                                    diff_saved,diff_wasted_minus,diff_walecznosc_3,combined_form_chart,home_cumulative_elo,away_cumulative_elo)
                    if result_one:
                ...
                result_one = {}
                result_one["datetime"] = record["datetime"]
                result_one["home_name"] = record["home_name"]
                result_one["away_name"] = record["away_name"]
                result_one["winner"] = record["winner"]
                result_one["home_elo"] = elo_home
                result_one["away_elo"] = elo_away
                #miejsce na dodanie dodatkowych parametrow
                result_one['elo_diff'] = elo_diff
                result_one['elo_diff_scaled'] = elo_diff_scaled
                result_one['experience'] = experience
                ...
                """
                result_one['walecznosc_home'] = walecznosc_last_50_home
                result_one['walecznosc_away'] = walecznosc_last_50_away
                result_one['diff_walecznosc_home'] = {
                    "3+": diff_3_home,
                    "4+": diff_4_home,
                    "5+": diff_5_home,
                    "6+": diff_6_home,
                ...
                result_one['diff_walecznosc_away'] = {
                    "3+": diff_3_away,
                    "4+": diff_4_away,
                    "5+": diff_5_away,
                    "6+": diff_6_away,
...
def main():
    ...
    async def run_loop():
        while True:
            #await value_object.add_experience(value_object.betting_ai)
            #await value_object.process_matches(value_object.betting_ai, 1)  # Process next n hour matches
            #await value_object.update_finished_matches_value_two(value_object.betting_ai, value_object.betting_ai2 )  # Update finished matches
            await value_object.update_finished_matches_value_three(value_object.betting_ai3) 
