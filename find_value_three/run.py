...
LAST_SEEN_ID = ObjectId("67cda48d49c855c8b0811dda")
class FindValueTest(ValueEngine):
    def __init__(self):
        ...
        self.gb_model = joblib.load("nowy_model_home.pkl")
        self.gb_model_away = joblib.load("trained_model_xgb_away.pkl")
    ...
    def find_value_three(self, date_for_match, home, away, elo_home, elo_away, odd_home, odd_away,
        ...
        df_input_home = pd.DataFrame([data_dict_home])

        # 3. Przewidujemy prawdopodobieństwa: p0 = home, p1 = away
        proba_gb_home = self.gb_model.predict_proba(df_input_home)[0]
        p0_home, p1_home = proba_gb_home[0], proba_gb_home[1]
        ...
        # 2. Zamieniamy to na DataFrame z 1 wierszem
        df_input_away = pd.DataFrame([data_dict_away])

        # 3. Przewidujemy prawdopodobieństwa: p0 = home, p1 = away
        proba_gb_away = self.gb_model_away.predict_proba(df_input_away)[0]
        p0_away, p1_away = float(proba_gb_away[0]), float(proba_gb_away[1])
        ...
        bet = None
        if (p1_away >= self.threshold_away and odd_away >= 1.7 and elo_home <= elo_away - 35) or (p1_away >= self.threshold_away and odd_away >= 1.7 and elo_home < elo_away and total_h2h >= 7 and h2h_home_scaled <= 0.37):
            if (p1_away > 0.75):
                found1 = True
        if (p0_home >= self.threshold_home and odd_home >= 1.7 and elo_home  - 35 >= elo_away) or (p0_home >= self.threshold_home and odd_home >= 1.7 and elo_home > elo_away and total_h2h >= 7 and h2h_home_scaled >= 0.63):
            if (p0_home > 0.90):
                found2 = True
        ...
        if found1:
            return {
                ...
                "odd_home": odd_home,
                "odd_away": odd_away,
                "p0_home_prob": p0_home,
                "p1_away_prob": p1_away,
    ...
    async def process_matches(self, betting_collection, n):
        """
        ...
        for record in matches:
            ...
            #logging.info(f'currentID checking: {LAST_SEEN_ID}')
            ...
            if home_name_tt and away_name_tt:
                ...
                result_one = self.find_value_three(date_for_match, home_name_tt, away_name_tt, elo_home, elo_away, odd_home, odd_away,
                ...
                if result_one:
                    ...
                    result_one['created_time'] = datetime.now()

                    if result_one["p0_home_prob"] > 0.9 or result_one["p1_away_prob"] > 0.75:
                        result_one["big_value"] = 1
                    else:
                        result_one["big_value"] = 0
...
def main():
    ...
    LAST_SEEN_ID = value_object.get_last_inserted_id()
    async def run_loop():
        while True:
            await value_object.process_matches(value_object.betting_ai ,12)  # Process next n hour matches
