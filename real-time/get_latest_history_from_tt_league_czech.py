...
def get_sets_details(game_link):
    ...
    try:
        ...
        response = session.get(game_link, headers=headers, timeout=5)
        ...
        for link in links:
            ...
            for l, ft_res in enumerate(data_without_match_details[6]):
                if "3" not in ft_res:
                    ids_to_pop.append(l)
                    continue
            # sprawdzamy czy juz mecz nie zostal czasem scrapowany i zapisany do bazy...
            for l, games_link in enumerate(data_without_match_details[2]):
                if matches_coll.count_documents({ 'tt_url': games_link }):
                    if l not in ids_to_pop:
                        ids_to_pop.append(l)  
            # sprawdzamy czy mecz w ogole mogl juz sie odbyc, jesli nie to skip
            for l, date_match in enumerate(data_without_match_details[0]):
