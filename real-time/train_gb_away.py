...
import joblib
from xgboost import XGBClassifier  # <--- Importujemy XGBoost
import matplotlib
matplotlib.use('Agg')
...
class AITraining(ValueEngine):
    def __init__(self):
        super().__init__()
        self.model_path = "trained_model_xgb_away.pkl"  # finalny pipeline (scaler + model)
    ...
    def load_data(self):
        ...
        """
        data = pd.DataFrame(list(self.database_parameters_coll_away.find()))
        data = data[data["away_elo"] > data["home_elo"]]  # FILTR KLUCZOW
    ...
    def transform_features(self, data):
        ...
        flip_features = [
            'elo_diff_scaled', 'diff_last_tournaments', 'diff_zmeczenie',
            'diff_h2h', 'diff_players_vs_opponents_50', 'diff_dominance_percentage',
            'diff_closest_trends', 'diff_saved'
        ]
        ...
        if "h2h_home_scaled" in data.columns:
            data["h2h_away_scaled"] = 1 - data["h2h_home_scaled"]
    ...
    def run_training(self):
        ...
        prelim_pipeline = Pipeline([
            ...
            ("gb", XGBClassifier(
                ...
                random_state=42,
                n_estimators=300,      # Więcej drzew dla lepszej stabilności
                learning_rate=0.05,    # Niższa wartość dla lepszej generalizacji
                subsample=0.8,
