...
class ValueEngine:
    def __init__(self):
        ...
        self.betting_coll_two = self.db['betting_']
        self.matches_coll = self.db['matches']
        self.parameters_coll = self.db['parameters_test']
        self.betting_ai = self.db['betting_ai']
        self.database_parameters_coll = self.db['parameters_dev']
        self.database_parameters_coll_away = self.db['parameters_new']
        self.last_processed_match_id = None  # Keeps track of the last processed match  parameters_test
        #endregion

        # Load config
        self.config = self.load_config('real-time/.creds')
    ...
    def get_upcoming_matches(self, n, last_seen_id, create_features=False):
        """
        ...
        if not create_features: 
            ...
            query = {"_id": {"$gte": last_seen_id}} if last_seen_id else {"datetime": {"$gte": now.astimezone(self.utc_zone), "$lt": future_time.astimezone(self.utc_zone)}}
            # Pobierz mecze zaplanowane na kolejne n godziny
            matches = self.calendar_coll.find(query)
            return list(matches)
        else:
            future_time = now - timedelta(hours=n)
    ...
    async def send_message_value_two(self, message):
        ...
        if message["big_value"] == 0:
            ...
            formatted_message = (
                ...
                f"Home p={round(message['p0_home_prob'],2)}\n"
                f"Away p={round(message['p1_away_prob'],2)}\n"
                f"Diff last tour. {round(message['diff_last_tournaments'],2)}\n"
                f"Exp {round(message['experience'],2)}\n"
                f"Diff H2H {round(message['diff_h2h'],2)}\n"
                f"Total H2H {round(message['total_h2h'],2)}"
            )
        else:
            # Przygotowanie wiadomości tekstowej
            formatted_message = (
    ...
    async def update_finished_matches_value_two(self, betting_collection):
        ...
        betting_coll = betting_collection.find({"edited": False})
        bot = Bot(token=self.bot_token)
        ...
        for match in betting_coll:
            ...
            time_range_start = match_datetime - timedelta(minutes=260)
            time_range_end = match_datetime + timedelta(minutes=260)
            ...
            # Aktualizacja rekordu w `betting_collection`
            update_result = betting_collection.update_one(
                {"_id": match["_id"]},
                {"$set": {"ft_result": ft_result, "edited": True}}
            )

            if update_result.modified_count == 0:
                print(f"[WARNING] Could not update betting record for: {home_name} vs {away_name}.")
                continue

            print(f"[INFO] Betting record updated for match: {home_name} vs {away_name}. ft_result: {ft_result}")

            # Pobranie ID wiadomości z Telegrama
            telegram_message_id = match.get("telegram_message_id")
            if not telegram_message_id:
                print(f"[WARNING] No Telegram message ID found for match: {home_name} vs {away_name}. Skipping.")
                continue

            # Edycja wiadomości na Telegramie
            try:
                updated_message = (
                    f"📅 Data: {match['datetime']}\n"
                    f"🏆 Mecz: {home_name} 🆚 {away_name}\n"
                    f"🔢 ELO: {match['home_elo']} 🆚 {match['away_elo']}\n"
                    f"📈 Kursy: {match['odd_home']} 🆚 {match['odd_away']}\n\n"
                    f"🎯 Zakład: {match['bet']}\n\n"
                    f"🏅 Wynik setowy: {match_database_record['ft_result']}\n"
                    f"🏅 {match_database_record['result']}\n"
                    f"🏅 Zwyciezca: {winner}\n"
                    f"{'🟢🟢🟢' if int(winner) == int(match['bet']) else '❌❌❌'}"
                ...
                print(f"[INFO] Telegram message updated for match: {home_name} vs {away_name}.")
            except TelegramError as e:
                print(f"[ERROR] Failed to update Telegram message for match: {home_name} vs {away_name}. Error: {e}")
        #endregion
    ...
    def get_last_matches_of_player(self, player_name):
        ...
        query = {
            ...
            "datetime": {
                ...
                "$lt": datetime.combine(last_match_date, datetime.max.time())
            }
        }
    ...
    def analyze_last_50_matches_for_player(self, player_name):
        ...
        """
        matches = list(
            self.matches_coll.find({"$or": [{"home_name": player_name}, {"away_name": player_name}]})
            .sort("datetime", -1)
            .limit(50)
        )
    ...
    def calculate_elo_diff(self, player_a_name, player_b_name):
        ...
        try:
            ...
            if len(matches_a) == 0:
                print(f"[DEBUG] No matches found for player: {player_a_name}")
                return None
            if len(matches_b) == 0:
                print(f"[DEBUG] No matches found for player: {player_b_name}")
                return None
    ...
    def get_matches_history_of_player(self, player_name, match_datetime, days_back=365):
        ...
        try:
            ...
            if not raw_matches:
                print(f"[DEBUG] No matches found for player: {player_name} within timeframe.")
                return []
    ...
    def avg_opponents_last_50_matches(self, player_name):
        ...
        try:
            ...
            if not last_matches:
                print(f"[DEBUG] No matches found for player: {player_name}")
                return 0
