...
def match_players():
    ...
    dt_object = datetime.datetime.today() - datetime.timedelta(days=1)
...
# Określona godzina, o której kod ma być uruchomiony (np. 14:30)
target_time = "00:01"

# Funkcja, która sprawdza, czy nadszedł czas
def czekaj_na_czas(target_time):
    while True:
        current_time = datetime.datetime.now().strftime("%H:%M")  # Pobierz bieżący czas w formacie HH:MM
        
        if current_time == target_time:
            match_players()
        
        # Czekaj 60 sekund przed sprawdzeniem ponownie
        time.sleep(60)

# Uruchom funkcję, aby czekała na określoną godzinę
czekaj_na_czas(target_time)